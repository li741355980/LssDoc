...
public final class SoLibManager {
                ...
                if (zipEntryName.endsWith(".so") && zipEntryName.contains(cpuArchitect)) {
                   // Log.e("TestSolib","----"+zipEntryName);
                    ...
                    if (lastModify == DLConfigs.getSoLastModifiedTime(context, zipEntryName)) {
                        ...
                        continue;
                    }//这里因为异步处理复制，而他里面的处理是，复制完一个文件立即关闭zip,所以会有异步问题
     ...
     * @author mrsimple
     */
    private static HashMap<ZipFile,Integer> mTaskZipFiles=new HashMap<ZipFile, Integer>();
    private class CopySoTask implements Runnable {
        ...
        CopySoTask(Context context, ZipFile zipFile, ZipEntry zipEntry, long lastModify) {
            Integer iret=mTaskZipFiles.get(zipFile);
            if(iret==null){
                mTaskZipFiles.put(zipFile,1);
            }else{
                mTaskZipFiles.put(zipFile,iret+1);
            }
            mZipFile = zipFile;
            mContext = context;
            mZipEntry = zipEntry;
        ...
        private void writeSoFile2LibDir() throws IOException {
            ...
            //Log.e("TestSo"," from "+mSoFileName+" to "+sNativeLibDir);
            copy(is, fos);
            int iret=mTaskZipFiles.get(mZipFile);
            if(iret==1){
                mZipFile.close();
                mTaskZipFiles.put(mZipFile,0);
            }else{
                mTaskZipFiles.put(mZipFile,iret-1);
            }
        }
        ...
        public void copy(InputStream is, OutputStream os) throws IOException {
            ...
            int size =1024;// getAvailableSize(bis);
            byte[] buf = new byte[size];
            int i = 0;

            while ((i = bis.read(buf, 0, size)) != -1) {
                //Log.e("TestSo","write "+i);
                bos.write(buf, 0, i);
            }
            bos.flush();
            bos.close();
